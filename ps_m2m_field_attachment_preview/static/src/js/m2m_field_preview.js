...
class M2MImagePreviewDialog extends Component {
    static template = xml`
        <Dialog title="'Image Preview'">
            ...
            <t t-set-slot="footer">
                <button class="btn btn-secondary me-2" t-on-click="downloadFile">Download</button>
                <button class="btn btn-primary" t-on-click="() => props.close()">Close</button>
            </t>
        </Dialog>
    `;
    static components = { Dialog };
    static props = ["close", "imageUrl", "fileName"];

    downloadFile() {
        const link = document.createElement('a');
        link.href = this.props.imageUrl;
        link.download = this.props.fileName || "downloaded_image";
