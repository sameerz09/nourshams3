...
class CrmLead(models.Model):
    ...
    design_ids = fields.Many2many(
        ...
        relation='crm_lead_design_rel',  # New unique relation table name
        column1='crm_lead_id',  # Column for crm.lead IDs
    ...
    project_ids = fields.Many2many(
        ...
        relation='crm_lead_project_rel',  # New unique relation table name
        column1='crm_lead_id',  # Column for crm.lead IDs
    ...
    date_sit = fields.Date(string="Date Sit", tracking=True, required=True)
    ...
    def write(self, vals):
        ...
        STAGE_EDIT_GROUPS = {
            ...
            'pto': ['lytegen_contact_details.group_change_coordinators',
                    'lytegen_contact_details.group_account_coordinators'],
            'change_order': ['lytegen_contact_details.group_change_coordinators',
                             'lytegen_contact_details.group_ntp_coordinators',
                             'lytegen_contact_details.group_account_coordinators'],
            'retention': ['lytegen_contact_details.group_change_coordinators',
                          'lytegen_contact_details.group_ntp_coordinators',
                          'lytegen_contact_details.group_account_coordinators'],
            'scalation': ['lytegen_contact_details.group_change_coordinators',
                          'lytegen_contact_details.group_ntp_coordinators',
                          'lytegen_contact_details.group_account_coordinators'],
        }
        ...
        ACCOUNT_COORDINATOR_STAGES = ['pto', 'post_install_work', 'installed', 'install_ready', 'updates',
                                      'change_order', 'retention', 'scalation']
    ...
    #     """ Allow only users in 'group_user_role_auditor' to create CRM records. """
    #     if not self.env.user.has_group('lytegen_contact_details.group_user_role_auditor'):
    #         _logger.warning(f"User {self.env.user.id} attempted to create a CRM Lead but was restricted.")
    #         raise AccessError("You are not allowed to create CRM records.")
    #     return super(CrmLead, self).create(vals)
    #
    # def write(self, vals):
    #     """ Allow only users in 'group_user_role_auditor' to update CRM records. """
    #     if not self.env.user.has_group('lytegen_contact_details.group_user_role_auditor'):
    #         _logger.warning(f"User {self.env.user.id} attempted to update a CRM Lead but was restricted.")
    #         raise AccessError("You are not allowed to modify CRM records.")
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(CrmLead, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
                                                   submenu=submenu)

        if view_type == 'tree':
            if self.env.user.has_group('base.group_system'):
                # Admin user → allow Create
                res['arch'] = res['arch'].replace('create="1"', 'create="0"')
            else:
                # Normal users → disable Create
                res['arch'] = res['arch'].replace('create="1"', 'create="0"')
